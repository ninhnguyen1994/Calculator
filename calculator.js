...
// Thực hiện các hành động bên trong khi click vào các nút MC
...
$('.box-button button.del').click(function() {
        var value = $(".box-calculation").val();
        if (!(parseInt(parseFloat(value)) == 0 && value.length == 1))
            $(".box-calculation").val(value.slice(0, value.length - 1));
        if (value.length == 1)
            $(".box-calculation").val("");
    });　// xoá đi 1 chữ số trong ô phép tính
